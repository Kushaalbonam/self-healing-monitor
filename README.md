...
Autonomous Service Recovery System using Prometheus metrics and Python-based Docker orchestration to minimize MTTR.

This project demonstrates a production-grade Site Reliability Engineering (SRE) and SDET workflow. It ensures high availability by automatically healing crashed services and prevents performance regressions using CI/CD gates.
...
Observe: Prometheus scrapes /metrics from the Node.js app every 5 seconds.

Evaluate: A background Python Watchdog queries the Prometheus API for the up metric.

Act: If up == 0 (indicating a hang or crash), the watchdog executes a docker restart command.

Verify: GitHub Actions runs a 10-user load test on every push to ensure 95% of requests stay under 250ms.
